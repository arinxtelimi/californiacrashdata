...
crashes = read_csv(
  ...
  filter(ACCIDENT_YEAR >= 2021 & ACCIDENT_YEAR <= 2023)
...
server <- function(input, output) {
  ...
  filtered_crashes = reactive({
    ...
    crashes |>
      filter(
        ...
        if (input$po_name != "All Cities") str_to_upper(PO_NAME) == str_to_upper(input$po_name) else TRUE
      )
  })
  ...
  map_filtered_crashes = reactive({
    ...
    if (str_to_upper(input$map_city) %in% str_to_upper(crashes$PO_NAME)) {
      map_data = map_data |>
        filter(str_to_upper(PO_NAME) == str_to_upper(input$map_city))
    } else {
      map_data = map_data |>
        filter(str_to_upper(PO_NAME) == "BERKELEY")
    }
  ...
  output$plot1 <- renderPlotly({
    ...
    myplot = plot_data |>
      ...
      geom_col() +
      facet_wrap(~ COLLISION_SEVERITY, scales = "free_y", ncol = 2) +
      labs(
        title = paste("Crashes by", display_names[[input$plot1_x]], "and Severity"),
      ...
      theme(
        ...
        axis.text.y = element_text(angle = 15, size = 8),
    ...
    ggplotly(myplot, tooltip = "text") |>
      layout(
        title = list(
          text = paste("Crashes by", display_names[[input$plot1_x]], "and Severity"),
          font = list(size = 14),
          yref = "paper",
          y = 1.1
        )
